...
- Fixed expanded state when after device`s rotation banner was shown in wrong size.
...
- Performance updates.
- Update request builder when IDFA wasn`t sent.
...
- Update request builder when idfa wasn`t sent in situation when advertisingTrackingEnabled property was YES. 
...
- Fixed adView when observers isn`t removed after deleting view.
- Performance updates.
